...
const Button = React.forwardRef(({ className, variant, icon, logoStyles, size, children, asChild = false, ...props}, ref) => {
    ...
    return (
        <button className={cn(buttonVariants({ variant, size, className }))} ref={ref} {...props}>
            {icon && (<Image src={icon} className={(twMerge('mr-2'), logoStyles)} alt='icon' width={30} />)}
            {children}
        </button>
    )
});
